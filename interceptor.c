...
asmlinkage long interceptor(struct pt_regs reg) {
	...
	// if some pid monitored, check if pid is being monitored for particular syscall
	if(table[reg.ax].monitored == 1){
		if(check_pid_monitored(reg.ax, current->pid)){
			// log message and calls the original syscall
...
//	 }
	...
	if (cmd == REQUEST_SYSCALL_INTERCEPT) {
		...
		// set syscall table as read-write
		set_addr_rw((unsigned long)sys_call_table);
		// replace syscall with our interceptor
    ...
    //If the command is REQUEST_START_MONITORING or REQUEST_STOP_MONITORING
	// Arjun, by the looks of this right now, I urge you to read exactly what these 2 commands do.
	// AND REMEMBER TO USE THE SPINLOCKS WHENEVER YOU MODIFY THE TABLE STRUCT OR THE SYS_CALL_TABLE
    ...
    int res;
	 ...
	 if(pid != 0){
		 table[syscall].monitored = 1;
		 printk(KERN_DEBUG "result: Result %d , PID %d", res, pid);
		 return res;
	 }
}
...
static int init_function(void) {
	...
	int i;
	orig_custom_syscall = sys_call_table[0]; // stores original MY_CUSTOM_SYSCALL into orig_custom_syscall
	orig_exit_group = sys_call_table[__NR_exit_group]; // store original exit group into orig_exit_group
	spin_lock(&calltable_lock);
	set_addr_rw((unsigned long)sys_call_table);	// sets sys_call_table to read write
	sys_call_table[0] = my_syscall;			// hijacks MY_CUSTOM_SYSCALL to point to our custom syscall
	sys_call_table[__NR_exit_group] = &my_exit_group; 	// hijacks exit_group to point to our custom exit_group (STILL HAVE TO IMPLEMENT MY_EXIT_GROUP)

	set_addr_ro((unsigned long)sys_call_table); 	// sets sys_call_table to read only
	spin_unlock(&calltable_lock);			// unlock

	// initialize the table struct values
	spin_lock(&pidlist_lock);
